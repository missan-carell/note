...
该案例为综合案例，有对模块，oopz，以及数据容器的理解和使用，可参考：

[第五章：函数和数据容器](#第五章：函数和数据容器)

[第七章：模块与包](#第七章：模块与包)

[第八章：面对对象编程 OOP](#第八章：面对对象编程 OOP)

1. 基于模块实现租房系统

```py
# main.py
from house_opertion import *
...
# 在案例中的临时数据
house_list =[{'id':1,'name':'tim','phone':123,'ad':'上海','cost':800,'state':'已出租'},
             {'id':2,'name':'jam','phone':234,'ad':'上海','cost':900,'state':'未出租'}]

# house_list = []

# 全局变量id_counter 用于记录当前的id
id_counter = 0
...
# 注意：这里添加的房屋列表会在重启系统后删除，如需要保存，可参考 快递管理 中保存到本地文件，或使用数据库
def list_house():
    ...
    if len(house_list) == 0:
        ...
        for house in house_list: # 从列表中取出字典
            for value in house.values(): # 从字典中取出值
                print(value,end="\t\t") # end 使输出在同一行，且输出后空格间隔
            print() # 输出完一个房屋信息后换行
...
def add_house():
    ...
    house = {'id':id_counter,'name': name, 'phone': phone, 'ad': ad, 'cost': cost, 'state': state}
    # 把新增的字典加入全局变量
    house_list.append(house)
    print("房屋添加成功！".center(16,"="))

def find_house(find_id):
...
def find_house_id():
    print("查找房屋信息".center(16,"="))
    house_id = int(input("要查找的房屋id:"))
    house = find_house(house_id)
    if house:
        print("编号\t\t房主\t\t电话\t\t地址\t\t月租\t\t状态（已出租/未出租）")
        for value in house.values(): # 因为现在house就是列表内对应id的字典，所以可以直接遍历
            print(value,end="\t\t")
        print()
    else:
        print("未找到目标编号！")
...
def del_house():
    del_id = int(input("请输入要删除房屋的编号:(-1退出）"))
    if del_id == -1:
        print("放弃删除房屋信息".center(16,"="))
        return


    if read_confirm("确认要删除房屋信息吗？(Y/N):").lower() == "y":
        house = find_house(del_id)
        if house : # 如果house不为None，对应的布尔值就是true
            house_list.remove(house)
            print(f"删除{del_id}号房屋成功".center(16, "="))
...
def change_info():
    info = int(input("请选择要修改的房屋编号:(-1表示退出）"))
    if info == -1:
        print("退出修改".center(16,"="))
        return
    else:
        house = find_house(info)
        if house :
            change_method('name',"房主",house)
            change_method('phone',"电话",house)
            change_method('ad',"地址",house)
            change_method('cost',"月租",house)
            change_method('state',"状态",house)

            # house['name'] = input("请输入姓名:")
            # house['phone'] = int(input("请输入电话:"))
            # house['ad'] = input("请输入地址:")
            # house['cost'] = int(input("请输入房租:"))
            # house['state'] = input("请输入出租状态:")
            # print("修改房屋信息成功！".center(16,"="))
...
def change_method(input_method,name_d,dict_name):
    new_value = input(f"将{name_d}修改为:")
    if len(new_value) > 0:
...
6. `list.sort()`自动排序（默认从小到大）
   ...
   - reverse: True表逆序（从大到小），默认是False（从小到大）
...
​	也可以key处填入匿名函数以实现特定的排序效果，详情可见[12.应用练习](#12.应用练习)
...
3. ==字典的遍历：==
   ...
   也可以通过 `dict.values`来直接获取字典的值

   ```py
   house = {'num':1,'name':'tim','phone':123,'ad':'上海','cost':800,'state':'已出租'}
   for value in house.values(): # 从字典中取出值
       print(value,end="\t\t") # end 使输出在同一行，且输出后空格间隔
   print() # 输出完一个房屋信息后换行
   ```
...
10. 从列表中的字典中获取值(value)：

    ```py
    house_list =[{'num':1,'name':'tim','phone':123,'ad':'上海','cost':800,'state':'已出租'},
                 {'num':2,'name':'jam','phone':234,'ad':'上海','cost':900,'state':'未出租'}]
    ...
    1		tim		123		上海		800		已出租		
    2		jam		234		上海		900		未出租		
    ====房屋列表显示完毕====
...
- 如果输出结果需要在列表中一起输出，需要用循环从列表中一个个取出
...
- sort详见：[2. 8 列表的常用命令](#2. 8 列表的常用命令)
